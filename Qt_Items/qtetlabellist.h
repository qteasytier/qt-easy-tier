...
class QtETLabelListItem : public QListWidgetItem
{
public:
    explicit QtETLabelListItem(QListWidget *parent = nullptr, int type = Type);
    explicit QtETLabelListItem(const QString &text, QListWidget *parent = nullptr, int type = Type);
    explicit QtETLabelListItem(const QIcon &icon, const QString &text, QListWidget *parent = nullptr, int type = Type);
    explicit QtETLabelListItem(const QtETLabelListItem &other);
    ~QtETLabelListItem() override = default;

    QtETLabelListItem &operator=(const QtETLabelListItem &other);
};

/// @brief 自定义列表视图委托
/// 负责绘制列表项的高亮效果和动画
class QtETLabelListDelegate : public QStyledItemDelegate
{
    Q_OBJECT

public:
    explicit QtETLabelListDelegate(QObject *parent = nullptr);
    ~QtETLabelListDelegate() override = default;
...
/// 仅用于标签项的展示
class QtETLabelList : public QListWidget
{
    Q_OBJECT
...
private:
    QtETLabelListDelegate *m_delegate;     ///< 自定义委托
