...
- `src/app_service`: app-service layer bridging UI and basic services: config commands/import-export, `VpnRuntimeService` (the application-level VPN runtime coordinator owning instance lifecycle, heartbeat sync, and `NodeInfoModel`/`RuntimeLogModel`, which live in `app_service/runtime/`), settings (`SettingsStore`/`SettingsBackendService`), autostart, logs, `DangerousOperationService`, favorite import/export. `DangerousOperationService` is a QML-facing service registered under the compatibility name `DangerousOperationViewModel`. Links `qtet_vpn`; basic services must NOT include `app_service` or `viewmodels` headers.
