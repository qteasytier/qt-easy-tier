...
Qt 6.5+ / C++20 / QML 桌面应用；C++ 承担业务和持久化，QML 主要做展示与绑定。QML 模块 URI 是 `QtEasyTier`，入口在 `src/main.cpp`，由 `engine.loadFromModule("QtEasyTier", "Main")` 加载。
...
- 单测可聚焦运行：`ctest --test-dir build -R tst_network_conf --output-on-failure` 或 `./build/tests/tst_network_conf`。
...
- 根 `CMakeLists.txt` 定义生产模块：`qtet_log`、`qtet_config`、`qtet_repository`、`qtet_service`、`qtet_platform`、`qtet_application`、`qtet_vpn`、`qtet_viewmodel`、`qtet_appsupport`。
...
- `src/app` 是装配层：`AppServices` 创建并持有服务/ViewModel，`QmlSingletonRegistrar` 集中 `qmlRegisterSingletonType`。
- `src/core/config`：`NetworkConf`、TOML 序列化、校验。
...
- `src/core/util` 放 `AutoStartHelper`、`FontHelper`、`LogHelper` 等工具；当前没有 `src/core/platform/` 目录，`qtet_platform` target 编译的是 `src/core/util/AutoStartHelper.cpp` 和 `FontHelper.cpp`。
...
- 测试注册在 `tests/CMakeLists.txt`，当前目标：`tst_network_conf`、`tst_sqlite_repository`、`tst_config_list_model`、`tst_favorite_node_repository`、`tst_daemon_client`、`tst_autostart_helper`、`tst_settings_store`、`tst_autostart_service`、`tst_public_server_provider`、`tst_import_nodes_viewmodel`、`tst_app_services`、`tst_log_repository`、`tst_log_helper`。
- 新测试使用 `add_core_test(name file.cpp)`，再按职责链接 `qtet_*` 模块 target。

## 仓库杂项

- `.gitignore` 忽略 `/build*/`、`/.qtcreator/`、`/.idea/`、`/.opencode/`、`/.worktrees/`、`/example/` 和运行期 `configs*` 数据库文件；不要把这些作为源码提交。
