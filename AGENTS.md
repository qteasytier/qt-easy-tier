...
- New QML files must be registered in the CMakeLists.txt of their owning directory under `src/qml/` (`components/`, `components/config_form/`, `pages/`, `dde/`); the root `CMakeLists.txt` aggregates these lists (`QTET_QML_FILES`, plus `QTET_QML_MASK_SHARED`/`QTET_QML_MASK_DDE` for the DDE-masked set). List entries are paths relative to `src/qml/` and double as `QT_RESOURCE_ALIAS` (so qrc paths stay `qrc:/QtEasyTier/<relative path>`). New non-QML resources must be added to `assets/resources.qrc`.
...
- `NetworkOptions.qml` is a data-driven thin shell: `ConfigEditorViewModel::formSections` (CONSTANT metadata: card groups, field key/title/type, combo options, spin ranges) is rendered by `FormField.qml` which dispatches to per-type renderers under `src/qml/components/form/`; values are read via `ConfigEditorViewModel[fieldKey]` and written via `setFieldValue(key, value)` (QMetaObject reflection onto the named properties — all named setters/tests stay authoritative). Field-interlock disabling (dhcp→ipv4, whitelist switch→its text field) is deliberately QML-side (`fieldEnabledByKey` in the page shell). DDE/DTK renderers can later replace the `components/form/` set via the same-name masking mechanism. `tst_network_options_smoke` loads the shell QML with a real `AppServices` assembly — note `AppServices` conditionally creates config-related ViewModels, so pass a real opened `DatabaseConnection` (an invalid `QSqlDatabase` yields null singletons).
...
- `Card.qml` uses `contentSpacing`; do not add/rename it to `spacing` because `Frame.spacing` is FINAL in Qt 6.7+. The shared (non-DDE) `Card`/`IconToolButton`/`QtETabWidget`/`ErrorDialog` are Swb-based but keep their original external API (`borderColor`, `iconSource`/`iconTint`, `tabTitle` auto-registration); pilot pages `SettingsPage.qml` and `LogPage.qml` use Swb controls directly.
- Prefer `palette.*` colors in QML; status colors come from `Theme.qml` (`statusGreen`, `statusOrange`, `statusRed`, `statusBlue`). Pages/components restyled with SwbControls use `SwbTheme` tokens (`background`/`popover`/`foreground`/`mutedForeground`/`border`/`secondary`) instead of `palette.*` — do not mix the two systems inside one file.
