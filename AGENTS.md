...
- Root `CMakeLists.txt` defines the app target, output dirs, and daemon options, and aggregates the QML file lists collected per-directory under `src/qml/` (`QTET_QML_FILES` plus the `QTET_QML_MASK_SHARED`/`QTET_QML_MASK_DDE` mask sets); production modules each have their own `src/**/CMakeLists.txt`. No `QQuickStyle` is injected from C++ anymore — all shared UI is SwbControls-based and carries its own `SwbTheme`.
