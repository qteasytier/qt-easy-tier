...
- DDE/DTK frontend conventions (everything under `src/qml/dde/`): **never import `QtQuick.Controls`/QQC** — every control comes from `org.deepin.dtk` (`ListView`, `TextArea`, `Label`, `ScrollBar`, … all exist there; plain `ListView` in a file that also imports `QtQuick` resolves to the DTK type). Only non-DTK imports allowed are `QtQuick`, `QtQuick.Layouts`, `QtQuick.Dialogs` (native `FileDialog`, which opens the system DDE portal dialog), and `QtEasyTier`:
  - Colors come from `palette.*` plus the shared `Theme.qml` status colors; never mix `SwbTheme` into a DDE file. Tabs are the self-drawn `dde/components/TabHeader.qml` (underline style) + `StackLayout` — do not use `TabBar` or Chameleon style injection.
  ...
  - Scrolling uses `Flickable` + DTK `ScrollBar.vertical: ScrollBar {}`; multiline text uses DTK `TextArea` (override `background` for a custom border, or `background: null` for plain log text); lists use DTK `ListView` (delegate `ListView.view` attached properties still work since DTK `ListView` subclasses `QQuickListView`).
