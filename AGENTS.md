...
- `src/core/application`: app-service layer bridging UI and basic services: config commands/import-export, `VpnRuntimeService` (owns `NodeInfoModel`/`RuntimeLogModel`, which live in `application/runtime/`), settings (`SettingsStore`/`AutoStartService`/`SettingsBackendService`), autostart, logs, `DangerousOperationService`, favorite import/export. Links `qtet_vpn`; basic services must NOT include `core/application` or `core/viewmodel` headers.
- `src/core/viewmodel`: QML-facing facades and Qt models. QML and ViewModels should call application services, not repositories/daemon clients/VpnManager directly (known exceptions: `LogViewModel`→`LogRepository`, `FavoriteNodeViewModel`→`FavoriteNodeRepository`, `BackendStatusViewModel`→`DaemonClient`, `ImportNodesViewModel`→`FavoriteNodeJsonCodec` — historical thin-shell debt, don't expand).
