...
- Windows 10/11
- 今后计划支持 Linux
- macOS 测试支持（社区测试包，未公证；首次使用 TUN 网络时需要管理员授权）
...
```
macOS 社区测试包请从仓库根目录单独执行，并显式开启临时 helper，否则生成的 app 不能作为 community DMG 打包：
```bash
cmake -S . -B build-macos-community -DCMAKE_BUILD_TYPE=Release -DCMAKE_OSX_ARCHITECTURES=arm64 -DQTEASYTIER_ENABLE_PROTOTYPE_HELPER=ON
cmake --build build-macos-community --config Release
cmake --install build-macos-community --config Release
MACOS_COMMUNITY_BUILD=1 bash package/mac/build_mac.sh "$(pwd)"
```

### 简单开始使用

#### macOS 测试包说明
macOS 预编译包目前按社区测试包发布，未使用 Apple Developer ID 公证。
如果系统阻止打开，可右键点击 `QtEasyTier.app` 选择“打开”，或在终端中对解压/安装后的 app 执行：
```bash
xattr -dr com.apple.quarantine /Applications/QtEasyTier.app
```
首次启动需要创建 TUN 网络时，QtEasyTier 会请求管理员授权来启动临时辅助进程。
正式 Developer ID 签名、公证和系统后台辅助服务仍作为后续增强目标。
#### 常规使用
- Windows 双击 `QtEasyTier.exe` 启动应用程序
- macOS 将 `QtEasyTier.app` 拖入 `/Applications` 后启动应用程序
- 点击左下角加号创建网络配置
- 输入网络名称、用户名、密码等基本信息
- 配置高级选项（可选）
- 点击 "运行网络" 按钮即可启动网络连接
