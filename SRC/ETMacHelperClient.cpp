...
bool forcePrototypeHelper()
{
#ifdef Q_OS_MACOS
#if QTEASYTIER_ENABLE_PROTOTYPE_HELPER
    return qEnvironmentVariableIsSet("QTEASYTIER_FORCE_PROTOTYPE_HELPER");
#else
    return false;
#endif
#else
...
bool useSystemHelperRuntime()
{
    ...
    const ETMacHelperServiceRegistrationState serviceState = ETMacHelperServiceManager::queryRegistrationState();
    return serviceState.kind == ETMacHelperServiceRegistrationStateKind::Enabled;
#else
    return false;
#endif
}

bool useXpcTransport()
{
#ifdef Q_OS_MACOS
    return useSystemHelperRuntime();
#else
    return false;
#endif
}

QString helperRuntimeDir(std::string *errorMsg, bool systemRuntime)
{
#ifdef Q_OS_MACOS
    if (systemRuntime && geteuid() != 0) {
        return QStringLiteral("/var/run/qteasytier");
    }

    QDir dir(systemRuntime
        ? QStringLiteral("/var/run/qteasytier")
        : QStringLiteral("/tmp/qteasytier-%1").arg(getuid()));
    ...
    if (systemRuntime && geteuid() == 0) {
        chown(dir.absolutePath().toLocal8Bit().constData(), 0, 0);
    }
...
QString helperTokenFilePath(std::string *errorMsg, bool systemRuntime)
{
    const QString runtimeDir = helperRuntimeDir(errorMsg, systemRuntime);
...
{
#ifdef Q_OS_MACOS
    const QString runtimeDir = helperRuntimeDir(nullptr, useSystemHelperRuntime());
    if (runtimeDir.isEmpty()) {
...
{
    return QString::fromLatin1(kHelperServiceName);
}

QString currentTokenFilePath(std::string *errorMsg)
{
    return helperTokenFilePath(errorMsg, useSystemHelperRuntime());
}
...
{
#ifdef Q_OS_MACOS
    return !useSystemHelperRuntime() && prototypeHelperAllowed();
#else
    return false;
#endif
}
...
ETMacHelperState ETMacHelperClient::queryHelperState(std::string *errorMsg)
{
    ...
    if (!useXpcTransport() && !QFile::exists(currentSocketPath())) {
    ...
    std::string token;
    if (!useXpcTransport()) {
        token = helperToken(errorMsg, false);
        if (token.empty()) {
...
bool ETMacHelperClient::ensureHelperAvailable(std::string *errorMsg)
{
    ...
    if (serviceState.kind == ETMacHelperServiceRegistrationStateKind::RequiresApproval) {
        if (prototypeHelperAllowed()) {
            return ensurePrototypeHelperRunning(errorMsg);
        }
        if (errorMsg) {
            *errorMsg = "QtEasyTier helper registration is waiting for administrator approval in System Settings";
        }
        return false;
    }

    if (serviceState.kind == ETMacHelperServiceRegistrationStateKind::Enabled) {
        if (prototypeHelperAllowed()) {
            return ensurePrototypeHelperRunning(errorMsg);
        }
        if (errorMsg && errorMsg->empty()) {
            *errorMsg = state.errorMsg.empty()
                ? "QtEasyTier service-managed helper is not running"
                : state.errorMsg;
        }
        return false;
    }

    if (serviceState.kind == ETMacHelperServiceRegistrationStateKind::NotRegistered
        || serviceState.kind == ETMacHelperServiceRegistrationStateKind::NotFound) {
        if (prototypeHelperAllowed()) {
            return ensurePrototypeHelperRunning(errorMsg);
        }
        if (errorMsg) {
            *errorMsg = "QtEasyTier helper is not installed. Open Settings and use macOS TUN helper Install/Upgrade first.";
        }
        return false;
    }
    ...
    if (!prototypeHelperAllowed()) {
        if (errorMsg && errorMsg->empty()) {
            *errorMsg = state.errorMsg.empty()
                ? "QtEasyTier helper is not available"
                : state.errorMsg;
        }
        return false;
    }
...
bool ETMacHelperClient::ensurePrototypeHelperRunning(std::string *errorMsg)
{
    ...
    QString escapedSocketPath = QDir(helperRuntimeDir(errorMsg, false)).filePath(QStringLiteral("helper.sock"));
    escapedHelperPath.replace(QStringLiteral("'"), QStringLiteral("'\\''"));
    escapedSocketPath.replace(QStringLiteral("'"), QStringLiteral("'\\''"));

    QString escapedTokenPath = helperTokenFilePath(errorMsg, false);
...
#ifdef Q_OS_MACOS
    if (useXpcTransport()) {
        return ETMacHelperXpcSendRequest(currentServiceName(), requestData, responseData, errorMsg);
    }
