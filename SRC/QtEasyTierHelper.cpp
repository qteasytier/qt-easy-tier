...
constexpr const char *kMainAppCodeIdentifier = "io.github.qteasytier.QtEasyTier";
constexpr const char *kHelperServiceName = "io.github.qteasytier.QtEasyTier.Helper";
...
bool preparePathParent(const QString &path, uid_t ownerUid, bool hasOwnerUid)
{
    ...
    const bool systemRuntime = dirPath == QStringLiteral("/var/run/qteasytier");
    if (systemRuntime && geteuid() == 0) {
        chown(dirPath.toLocal8Bit().constData(), 0, 0);
        chmod(dirPath.toLocal8Bit().constData(), 0711);
    } else {
        chmod(dirPath.toLocal8Bit().constData(), 0700);
    }

    if (hasOwnerUid && !systemRuntime) {
...
std::string cfStringToString(CFStringRef value)
{
    ...
    if (CFStringGetCString(value, stackBuffer, sizeof(stackBuffer), kCFStringEncodingUTF8)) {
        return stackBuffer;
    }
...
std::string currentTeamIdentifier()
{
    ...
    const OSStatus status = SecCodeCopySigningInformation(selfCode, kSecCSSigningInformation, &info);
    CFRelease(selfCode);
    ...
    if (teamValue && CFGetTypeID(teamValue) == CFStringGetTypeID()) {
        result = cfStringToString(static_cast<CFStringRef>(teamValue));
    }
    CFRelease(info);
    return result;
}

std::string createMainAppRequirementString(const std::string &teamIdentifier)
{
    if (teamIdentifier.empty()) {
        return {};
    }

    return "identifier \"" + std::string(kMainAppCodeIdentifier)
...
public:
    ...
    bool listenXpc(const QString &serviceName, const std::string &clientRequirement, std::string *errorMsg)
    {
        m_requiresClientCodeSignature = !clientRequirement.empty();
        return ETMacHelperXpcStartServer(
            serviceName,
            clientRequirement,
            [this](const QByteArray &requestData) {
                return responseData(handleRequest(requestData));
            },
            errorMsg);
    }
...
private:
    ...
    QJsonObject handleRequest(const QByteArray &requestData)
    {
        ...
        if (command == QStringLiteral("shutdown")) {
            if (!m_allowShutdown) {
                return errorResponse(QStringLiteral("Helper shutdown is not allowed for service-managed helper"));
            }
    ...
    QJsonObject handleStatus() const
    {
        ...
        response.insert(QStringLiteral("client_identity_required"), m_requiresClientCodeSignature);
        return response;
    }
    ...
    bool m_requiresClientCodeSignature = false;
...
int main(int argc, char *argv[])
{
    ...
    const QString xpcServiceName = argumentValue(arguments, QStringLiteral("--xpc-service"));
    ...
    if ((socketPath.isEmpty() && xpcServiceName.isEmpty())
        || (xpcServiceName.isEmpty() && tokenFilePath.isEmpty())) {
        std::cerr << "QtEasyTierHelper missing --socket/--xpc-service or --token-file" << std::endl;
        return 2;
    }
    ...
    QString token;
    const bool requireToken = xpcServiceName.isEmpty();
    if (requireToken) {
#if !QTEASYTIER_ENABLE_PROTOTYPE_HELPER
    ...
    HelperServer server(socketPath, tokenFilePath, token, ownerUid, hasOwnerUid, allowShutdown, requireToken);
    if (!xpcServiceName.isEmpty()) {
        const std::string teamIdentifier = currentTeamIdentifier();
        if (teamIdentifier.empty()) {
            std::cerr << "QtEasyTierHelper XPC service requires a signed helper with a Team ID" << std::endl;
            return 2;
        }
        const std::string requirement = createMainAppRequirementString(teamIdentifier);
        if (requirement.empty()) {
            std::cerr << "QtEasyTierHelper cannot create XPC client signing requirement" << std::endl;
            return 2;
        }
        std::string xpcError;
        if (!server.listenXpc(xpcServiceName.isEmpty() ? QString::fromLatin1(kHelperServiceName) : xpcServiceName, requirement, &xpcError)) {
            std::cerr << "QtEasyTierHelper failed to listen on XPC service: " << xpcError << std::endl;
            return 1;
        }
    } else {
        if (!server.listen()) {
